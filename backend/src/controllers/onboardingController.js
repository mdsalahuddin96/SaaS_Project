...
export const registerTenant = async (req, res) => {
  ...
  if (!subdomainRegex.test(cleanSubdomain)) {
    return res
      .status(400)
      .json({ error: "subdomain contain only lowercase, number and (-)" });
  }
  let newTenant;
  try {
    ...
    if (existingTenant) {
      return res
        .status(400)
        .json({
          error: "This subdomain already used by other.Try with new one!",
        });
    }
    ...
    res
      .status(500)
      .json({ error: "Server encountered an error. Filed Onboarding!" });
  }
};
